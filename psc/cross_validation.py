...
def parse_fold_id(fold_id):
    ...
    if len(fold_id_elements) > 1:
        ...
        error_msg = 'inner_fold_id {} is for fold #{}, which does not exist for outer fold_id {}'.format(fold_id,
                                                                                                         fold_idx_inner_cv,
                                                                                                         outer_fold_id)
...
def generate_cvindices(n_samples=None, strata=None, total_folds_for_cv=[1, 2, 3, 5, 10],
                       total_folds_for_inner_cv=[2, 3, 5], replicates=3, seed=None):
    """
    :param n_samples:
    :param strata:
    ...
    # type checks
    assert isinstance(total_folds_for_cv, list) and len(total_folds_for_cv) > 0 and (
                len(total_folds_for_cv) == len(set(total_folds_for_cv)))
    assert (total_folds_for_inner_cv is None) or (
                isinstance(total_folds_for_inner_cv, list) and len(total_folds_for_inner_cv) == len(
            set(total_folds_for_inner_cv)))
        ...
        for n in range(1, replicates + 1):
            ...
            for f in range(1, k + 1):
                ...
                for l in total_folds_for_inner_cv:
                    inner_fold_id = to_fold_id(total_folds=k, replicate_idx=n, fold_idx_inner_cv=f,
...
def validate_folds(folds, fold_id=None, n_samples=None, stratified=True):
    ...
    assert np.array_equal(fold_values, np.arange(1, fold_values_max + 1)), 'fold indices {} are not consecutive'.format(
        fold_values)

    if not stratified:
        assert np.min(fold_counts) >= np.max(
            fold_counts) - 1, 'imbalanced folds: max (points/fold) must be within min (points/fold)'
